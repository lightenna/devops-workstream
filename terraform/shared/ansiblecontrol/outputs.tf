...
output "instantiated_host_ssh_command" {
  value = "ssh -A -o StrictHostKeyChecking=no -o ProxyCommand='ssh -o StrictHostKeyChecking=no -W %h:%p centos@${var.bastion_host}' centos@${aws_instance.ansible_control_node.private_ip}"
}
