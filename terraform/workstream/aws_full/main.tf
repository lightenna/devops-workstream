...
module "puppetmless" {
  ...
  hostname = "puppetmless${local.unique_append}"
  region = var.aws_region

  # use the fields passed back from aws_background for guaranteed consistency
  host_os_image = module.aws_background.aws_ami_id
  bastion_public_ip = module.aws_background.aws_bastion_public_ip
  ssh_key_name = module.aws_background.aws_key_pair_id
  nsg_id = module.aws_background.aws_security_group_id
  subnet_id = module.aws_background.aws_subnet_id
  puppet_environment = "workstream"
}

# output command for accessing host by SSH
output "ssh_command_puppetmless" {
  value = module.puppetmless.ssh_command
}
...
output "ssh_command_puppetmaster" {
  value = module.puppetmaster.ssh_command
}
